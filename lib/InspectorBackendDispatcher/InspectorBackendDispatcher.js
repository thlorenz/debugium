...
function forwardEvents(self, agent) {
  if (typeof agent.on !== 'function') return;
  agent.on('message' , self.emit.bind(self , 'message'))
  agent.on('error'   , self.emit.bind(self , 'error'))
}
...
function InspectorBackendDispatcher(opts) {
  ...
  forwardEvents(this, opts.pageAgent)
